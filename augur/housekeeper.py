...
class Housekeeper:
    ...
    def updater_process(broker_host, broker_port, broker, job, logging_config):
        ...
        if 'repo_group_id' in job:
            ...
            logger.info(
                f"Housekeeper spawned {job['model']} model updater process for repo group id {repo_group_id}"
            )

        else:
            repo_group_id = None
            logger.info(
                f"Housekeeper spawned {job['model']} model updater process for repo ids {job['repo_ids']}"
            )
        ...
        try:
            ...
            while True:
                ...
                logger.info(
                    (
                        "Housekeeper recognized that the broker has a worker that "
                ...
                while True:
                    logger.info(
                        f"Housekeeper updating {job['model']} model with given {job['given'][0]}..."
                    )


                    if job['given'][0] in ['git_url', 'github_url']:
                        for repo in job['repos']:
                            ...
                            try:
                                requests.post(
                                    f'http://{broker_host}:{broker_port}/api/unstable/task',
                                    json=task,
                                    timeout=10,
                                )
                    ...
                    logger.info(
                        f"Housekeeper finished sending {len(job['repos'])} tasks to the broker for it to distribute to your worker(s)"
                    )
